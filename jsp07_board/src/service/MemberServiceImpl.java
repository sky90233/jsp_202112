...
//		String secretpw = sha256(member.getPasswd(),saltmake());
	...
	//평문을 암호문으로 변경(하는 메소드 만들기)
	public String sha256(String passwd,String salt) {
		//String 대신 사용, 속도가 빠르다
		StringBuffer sb = new StringBuffer();
		try {
			//SHA-256 :단방향 암호기법, 복호화 불가능 256bit(16진수 64자리)
			MessageDigest md =MessageDigest.getInstance("SHA-256");
			//passwd :String //getBytes : 문자열 byte배열로 바꾸어줌
			md.update(passwd.getBytes());//문자열 바이트 배열로 변경해서 전달
			md.update(salt.getBytes()); //솔트를 추가(해커들의 레이보우테이블 막아주는 대책)
	...
	public Map<String,String> login(String email, String passwd) {
		...
		if(member==null) {
			...
			//db에 저장된 비밀번호화 암호화된 비밀번호가 일치한다면
	...
	public String update(Member member, String changepw) {
		...
		if(dbpw.equals(secretpw)) {//변경권한 생김
			//비밀번호가 변경됐다면 
			if(!changepw.equals("")) {
				//솔트구하기
				...
				member.setPasswd(secretpw);
				member.setSalt(salt);	
			}else {//비밀번호가 변셩이 안됐다변 기존정보 다시 셋팅
